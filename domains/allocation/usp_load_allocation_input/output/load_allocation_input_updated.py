...
Place beside load_allocation_input.py in:
  Source/AllocationV2/usp_load_allocation_input/output/

Also copy step_timer_updated.py, checkpoint_updated.py, config_parallel_hooks.py,
parallel_config_updated.py, config_loaders_updated.py, and shared_views_updated.py
into the same output/ folder.

RUNNER Change 2:
  module_name = f"AllocationV2.{sp_name}.output.load_allocation_input_updated"
"""
...
from .config_loaders_updated import load_common_config_parallel, load_config_parallel
from .shared_views_updated import register_shared_views_parallel
...
from .ai_pfic_service import (
    build_pfic_snapshot, build_pfic_elections, build_pfic_allocation_input,
    apply_pfic_election_deletes, apply_part_v_vii_flags,
)
from .ai_pfic_flowup_service import (
    build_pfic_flowup_pipeline, build_custom_footnote_input, check_pfic_xml_override_alert,
)
from .ai_finalization_service import (
    ...
    apply_distribution_line_suppression, write_form_flowups, purge_output_tables,
)
...
def run_load_allocation_input(
    ...
    **kwargs,
    ...
    t0 = time.time()
    log_section("load_allocation_input_updated")
    ...
    with timer.step("checkpoint_pfic_snapshot"):
    ...
    with timer.step("checkpoint_alloc_input"):
    ...
    with timer.step("checkpoint_pfic_raw"):
    ...
    with timer.step("checkpoint_pfic_flowup"):
    ...
    with timer.step("checkpoint_alloc_filtered"):
    ...
    if cfg.get("investment_tag_workflow_id", 0) != 0:
        with timer.step("checkpoint_alloc_tagged"):
    ...
    timer.print_summary("load_allocation_input_updated")
    log_timing("load_allocation_input_updated", t0)
    ...
    return {
        ...
        "implementation": "load_allocation_input_updated",
