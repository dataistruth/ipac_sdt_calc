...
Kept as a module so relative imports (``from .plan_profiler import ...``) work
unchanged across the optimized Allocation SPs. All logic lives in the shared
package (deploys to ``Source/AllocationV2/plan_profiler``); this file only
forwards its public API and provides safe no-op fallbacks when the shared
...
try:
    ...
    def build_plan_profile_display(spark, records, threshold=30):
        return None
