"""Failure-safe shim over the shared AllocationV2 plan profiler."""
...
try:
    ...
    def track_plan(fn):
        """Add dict-result support while retaining shared no-op behavior."""
            ...
            if not isinstance(cfg, dict):
                cfg = next(
                    (
                        ...
                        for value in args
                        if isinstance(value, dict)
                        and ("run_id" in value or "profile_plan" in value)
                    ),
                    None,
                )
