"""Mutation-safe RunID reconciliation for the SP's output and input writes."""
...
def purge_run(spark, catalog, schema, run_id):
    """Delete only output rows; input is restored from a benchmark snapshot."""
    purged = []
...
def create_run_snapshot(spark, catalog, schema, table, run_id):
    """Create an isolated Delta copy before a benchmark mutates a table."""
...
def create_input_snapshot(spark, catalog, schema, run_id):
...
def restore_input_snapshot(spark, catalog, schema, run_id, snapshot_table):
...
drop_input_snapshot = drop_run_snapshot
