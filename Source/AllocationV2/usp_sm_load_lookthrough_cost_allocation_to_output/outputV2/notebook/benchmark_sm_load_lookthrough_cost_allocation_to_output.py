...
# MAGIC %md
# MAGIC # SM look-through cost allocation: production vs outputV2
# MAGIC
# MAGIC This benchmark snapshots both mutated RunID partitions, restores the
# MAGIC input before every variant, and restores the original state in `finally`.
...
dbutils.widgets.text("RankForRulePickup", "0", "8. Rule rank")
...
rank_for_rule = int(dbutils.widgets.get("RankForRulePickup") or "0")
...
PACKAGE = (
    "AllocationV2."
    "usp_sm_load_lookthrough_cost_allocation_to_output"
)
...
def _restore_baseline(snapshots):
    for table, snapshot in snapshots.items():
        reconcile.restore_run_snapshot(
            spark, catalog, schema, table, run_id, snapshot
        )
...
def _run_variant(variant, pass_number, snapshots):
    ...
    _restore_baseline(snapshots)
    reconcile.purge_run(spark, catalog, schema, run_id)
    kwargs = {
        "EntityID": entity_id,
        "ClientID": client_id,
        "TaxPeriodID": tax_period_id,
        "RunID": run_id,
        "RankForRulePickup": rank_for_rule,
        "CatalogName": catalog,
        "SchemaName": schema,
        "ResultType": result_type,
        "VolumePath": volume_path or None,
        "ExecutionID": f"sm-cost-ab-{pass_number}",
    }
    if variant == "updated":
        kwargs.update(
            {
    ...
    result = runner.run_sm_load_lookthrough_cost_allocation_to_output(
        spark, **kwargs
    )
    wall = round(time.perf_counter() - started, 3)
    outputs = reconcile.capture_outputs(spark, catalog, schema, run_id)
    profile = (
...
parity_rows = []
snapshots = {
    table: reconcile.create_run_snapshot(
    ...
    try:
        ...
        if restored:
            for snapshot in snapshots.values():
                reconcile.drop_run_snapshot(
                    spark, catalog, schema, snapshot
                )
        else:
            print(
                "[reconcile] restoration failed; retained snapshots:",
                snapshots,
            )
...
for item in checkpoint_profile:
    ...
    consumers = (
        1 if name.startswith("hier_level_")
        else consumer_counts.get(name)
    )
    recommendation_rows.append(
        {
            "pass": item["pass"],
            "name": name,
            "builder_delta": item.get("delta"),
            ...
            "rationale": (
                "Production seam retained. Accept or change only after "
                "alternating-order parity and measured checkpoint cost."
            ),
        }
    )
