...
Kept as a module so relative imports (``from .plan_profiler import track_plan``
/ ``plan_profile_report`` / ``measure_plan``) keep working unchanged. All logic
lives in the shared package ``Source/AllocationV2/plan_profiler``; this file
only forwards its public API,
with safe no-op fallbacks when the shared package is not deployed (plan
profiling is opt-in and non-essential, so a missing package must never break
the pipeline import).
"""
