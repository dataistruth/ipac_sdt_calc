...
def filter_asset_class_underlyings(
    ...
    probe_free = cfg.get(
        "_output_v3_asset_class_probe_removal", False
    )
    ...
    if override_flag != "C":
        ...
        matching1 = None
        if probe_free or not ear_null_tk.isEmpty():
            matching1 = (
                ac.alias("AI")
                .join(
        ...
        matching2 = None
        if probe_free or not ear_notnull_tk.isEmpty():
            matching2 = (
                ac.alias("AI")
                .join(
        ...
        # outputV3 folds the existence check into the final relation. This
        # preserves the production rule that an empty EAR table skips the
        # delete, without launching a third driver-side isEmpty() action.
        if probe_free or not entity_asset_class_rel.isEmpty():
            filtered_ac = (
                ac.alias("AI")
                .join(
                    enu_ut.alias("U"),
            ...
            if probe_free:
                ear_presence = F.broadcast(
                    entity_asset_class_rel.agg(
                        F.count(F.lit(1)).alias(
                            "_output_v3_ear_count"
                        )
                    )
                )
                # Preserve the original no-EAR path byte-for-byte: it skips
                # the ENU_UnderlyingType join and keeps every input row.
                # For a present EAR relation, apply the production filter.
                unfiltered_when_absent = (
                    ac.alias("AI")
                    .crossJoin(ear_presence)
                    .filter(F.col("_output_v3_ear_count") == 0)
                    .select("AI.*")
                )
                filtered_when_present = (
                    filtered_ac
                    .crossJoin(ear_presence)
                    .filter(
                        (F.col("_output_v3_ear_count") > 0)
                        & keep_row
                    )
                    .select("AI.*")
                )
                ac = unfiltered_when_absent.unionByName(
                    filtered_when_present
                )
            else:
                ac = filtered_ac.filter(keep_row).select("AI.*")
    else:
