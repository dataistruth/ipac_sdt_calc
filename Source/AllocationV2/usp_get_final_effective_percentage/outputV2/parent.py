"""Load production output modules without mutating their module objects."""
...
_PARENT_PACKAGE = (
    ...
    if __package__ and __package__.endswith(".outputV2")
    else ""
)
...
def output_module(short_name: str) -> ModuleType:
    """Import an unchanged helper from the production output package."""
    if not _PARENT_PACKAGE:
        raise ImportError("outputV2 must be imported from its AllocationV2 package")
    return importlib.import_module(f"{_PARENT_PACKAGE}.output.{short_name}")


def isolated_output_module(short_name: str) -> ModuleType:
    """Execute production source in a private outputV2 module namespace."""
